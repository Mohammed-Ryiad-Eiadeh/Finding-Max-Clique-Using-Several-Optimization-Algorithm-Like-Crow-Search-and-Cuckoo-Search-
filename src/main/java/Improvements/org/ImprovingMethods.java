...
public final class ImprovingMethods extends Feasibility {
    ...
    public double[] DoXor(double[][] solutions, FitnessFunction FN) {
        double[] parent01 = Arrays.copyOf(solutions[new Random().nextInt(solutions.length)], solutions[0].length);
        double[] parent02 = Arrays.copyOf(solutions[new Random().nextInt(solutions.length)], solutions[0].length);
        double[] offspring = new double[parent01.length];
        for (int i = 0; i < parent01.length ; i++) {
            offspring[i] = parent01[i] == parent02[i] ? 0.0 : 1.0;
        }
        double[] FeasibleOffspring = GuaranteeFeasibility(offspring);
        int OffspringFit = FN.EvaluateSingleSolution(FeasibleOffspring);
    ...
    public double[] DoSinglePointCrossOver(double[][] Solutions, FitnessFunction FN) {
        ...
        double[] x1 = FN.EvaluateSingleSolution(feasible1) > FN.EvaluateSingleSolution(parent1) ? feasible1 : parent1;
        double[] x2 = FN.EvaluateSingleSolution(feasible2) > FN.EvaluateSingleSolution(parent2) ? feasible2 : parent2;
