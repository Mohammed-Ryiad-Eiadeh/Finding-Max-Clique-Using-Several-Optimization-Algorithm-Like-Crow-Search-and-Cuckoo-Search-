...
class asdf {
    public static void main(String[] args) {
        System.exit(0);
        var linkedMap = new HashMap<String, Integer>(60, 0.35F) {{
            put("salma", 10);
            put("fadwa", 20);
            put("alia", 30);
        }};
        ...
        var map = linkedMap.
                ...
                collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue, Integer::compare, LinkedHashMap::new));

        map.entrySet().forEach(out::println);

        // sorting map by keys using list
       */
/* var l = new ArrayList<>(map.entrySet());
        Collections.shuffle(l);
        l.sort(Comparator.comparing(String::valueOf));
        var mmm = l.stream().collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue, (x, y) -> x, LinkedHashMap::new));

        out.println(mmm);*//*


        // sorting map by values using list
        */
/*var l = new ArrayList<>(map.entrySet());
        Collections.shuffle(l);
        l.sort(Map.Entry.comparingByValue());  // l.sort(Comparator.comparing(Map.Entry::getValue));  or  l.sort(Comparator.comparingInt(Map.Entry::getValue));
        var mmm = l.stream().collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue, (x, y) -> x, LinkedHashMap::new));
        var mmm = Map.ofEntries(l.toArray(Map.Entry[]::new)); // but this make key and values objects not their data types
        out.println(mmm);*//*


        // take a copy of the map
        // var ttt = Map.copyOf(map);

        // same as Map.of
        // var tt = Map.ofEntries(Map.entry(10, 20), Map.entry(20, 30));
...
class Test implements Comparable<Test> {
    ...
    public boolean equals(Object o) {
        ...
        Test Test = (Test) o;
        return x == Test.x && Double.compare(Test.y, y) == 0;
    }
