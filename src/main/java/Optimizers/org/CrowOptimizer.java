        ...
        for (int iter = 0; iter < maxIteration; iter++) {
            for (int solution = 0; solution < solutions.length; solution++) {
                if (new Random().nextDouble() >= AP) {
                    ...
                    solutions[solution] = super.GuaranteeFeasibility(Arrays.stream(solutions[solution]).map(V -> discretion.DiscreteViaTransfer(TF, V + uniformDistribution + FL * (folkMemory[randomCrow][ii.getAndAdd(1)]) - V)).toArray());
                } else
            ...
            for (int solution = 0 ; solution < solutions.length; solution++) {
                if (new Random().nextDouble() >= AP) {
                    ...
                    if (new Random().nextDouble() >= 0.5) {
                        ...
                        solutions[solution] = super.GuaranteeFeasibility(Arrays.stream(solutions[solution]).map(V -> discretion.DiscreteViaTransfer(TF, V + alpha * improvingMethod.Levy(iteration.get()))).toArray());
                    }
