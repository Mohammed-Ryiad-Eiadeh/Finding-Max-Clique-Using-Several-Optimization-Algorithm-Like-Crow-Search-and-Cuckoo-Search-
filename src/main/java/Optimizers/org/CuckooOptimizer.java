    ...
    public void StartOptimization() {
        ...
        for (int iter = 0; iter < maxIteration; iter++) {
            ...
            for (double[] doubles : solutions) {
                double[] evolvedSolution = GuaranteeFeasibility(Arrays.stream(doubles).map(ii -> TF.applyAsDouble(ii + stepSizeScaling * Math.pow(counter.get() + 1, -lambda))).toArray());
                ...
                if (new Random().nextDouble() < worstNestProbability) {
                    int r1 = new Random().nextInt(solutions.length);
                    int r2 = new Random().nextInt(solutions.length);
                    for (var j = 0; j < doubles.length; j++)
                        evolvedSolution[j] = TF.applyAsDouble(doubles[j] + delta * (solutions[r1][j] - solutions[r2][j]));
